...
class TrainingPollRoutine:
    ...
    def save_state(self, state):
        ...
        state[self.name] =\
            {
                'isEnabled': that_or_empty_string(self.isEnabled),
                'execDayNum': that_or_empty_string(self.executionDayNum),
                'channelId': that_or_empty_string(self.channelId),
                'lastExecDate':
                    "" if self.lastExecutionDate is None
                    else self.lastExecutionDate.isoformat(),
            }
...
def that_or_empty_string(value):
